...
const VideoNoteSection: React.FC<{ event: AuthorizedEvent }> = ({ event }) => !isSynced(event)
    ...
    : event.workflowStatus?.status !== "IDLE" && <Card kind="info">
        <Trans i18nKey={`manage.my-videos.workflow-status.${
            event.workflowStatus?.status === "BUSY" ? "active" : "unknown"
        }`} />
    </Card>;
...
const VideoButtonSection: React.FC<{ event: AuthorizedEvent }> = ({ event }) => {
    ...
    return <Inertable isInert={!isSynced(event) || event.workflowStatus?.status !== "IDLE"}>
        <ButtonSection>
...
const VideoMetadataSection: React.FC<{ event: AuthorizedEvent }> = ({ event }) => {
    ...
    return <MetadataSection
        disabled={event.workflowStatus?.status !== "IDLE"}
        {...{ commit, inFlight }}
        item={event}
    />;
};
