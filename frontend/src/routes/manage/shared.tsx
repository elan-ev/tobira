 ...
 */
export function createQueryParamsParser<ColumnType extends string, T>(
    parseColumnFn: (sortBy: string | null) => ColumnType,
    buildVars: (column: ColumnType, { limit, offset, direction }: QueryVars) => T,
) {
    return (queryParams: URLSearchParams): T => {
        const { limit, offset, direction } = parsePaginationAndDirection(queryParams);
        const sortBy = queryParams.get("sortBy");
        const column = parseColumnFn(sortBy);
        return buildVars(column, { direction, limit, offset });
    };
}
