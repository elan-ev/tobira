import {
    Card, Floating, FloatingHandle, match, screenWidthAbove, screenWidthAtMost, useColorScheme,
...
const SortingMenu: React.FC<SortingMenuProps> = ({ close, vars, sortOptions }) => {
    ...
    const list = <>
        ...
        <div>{t("manage.table.sorting.order")}</div>
        <ul role="menu" onBlur={handleBlur}>{sortDirections.map(direction =>
            <MenuItem
 ...
 */
export const TableRow = <T extends ListItemProps>({ item, ...props }: GenericListItemProps<T>) => {
    const { t } = useTranslation();
