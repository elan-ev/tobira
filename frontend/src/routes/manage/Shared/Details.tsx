...
export const DeleteButton = <TMutation extends MutationParameters>({
    ...
    const tKey = `${itemType}.${itemType}` as ParseKeys;
    ...
    const onSubmit = () => {
        commit({
            ...
            onError: error => {
                const failedAction = t("manage.shared.delete.failed");
    ...
    return <>
        <Button kind="danger" onClick={() => currentRef(modalRef).open()}>
            <span css={{ whiteSpace: "normal", textWrap: "balance" }}>
                {t("manage.shared.delete.title", { item: t(tKey) })}
            </span>
        </Button>
        <ConfirmationModal
            title={t("manage.shared.delete.confirm", { item: t(tKey) })}
            buttonContent={t("manage.shared.delete.title", { item: t(tKey) })}
            onSubmit={onSubmit}
            ref={modalRef}
        >
            <p>
