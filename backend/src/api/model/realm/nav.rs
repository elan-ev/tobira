...
pub(crate) struct RealmNav {
    // /// The topmost element of the nav, either the parent or grandparent of the
    // /// current page. For user root realms, this is the main root realm.
    // top: Option<RealmNavItem>,

    // /// The second element of the nav, between `top` and `list`. Is either the
    // /// current page or its parent. Is always `Some` if `top` is `Some`.
    // middle: Option<RealmNavItem>,
