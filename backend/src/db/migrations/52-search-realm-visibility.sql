-- Exposes `visible` and `flattened_moderator_roles` on `search_realms` so
-- that search results can exclude realms that are hidden from the current user.
...
create or replace view search_realms as
    select
        ...
        realms.visible,
        realms.flattened_moderator_roles
    from realms;


-- Identical to the `search_events` definition in `43-search-views-without-deletions.sql`.
...
-- Identical to the `search_series` definition in `47-search-thumbnail-info-with-state.sql`.
...
-- Identical to the `search_playlists` definition in `47-search-thumbnail-info-with-state.sql`.
